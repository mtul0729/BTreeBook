...
public:
	Book() = default;
	Book(int bid, std::string title, std::string author, int mount = 0) //默认mount为0,需要用setMount添加
	...
	bool operator==(const int bid) const {
		return this->bid == bid;
	}
	bool operator!=(const int bid) const {
		return this->bid != bid;
...
using INDEX_SIZE = std::vector<std::shared_ptr<BTreeNode>>::size_type; //结点的孩子数量类型,兼容书的数量类型,实际就是size_t
struct BTreeNode {  //2-3树（3阶B树）的节点
	std::vector<Book> books; //书
	std::vector<std::shared_ptr<BTreeNode>> children; //子节点
	std::weak_ptr<BTreeNode> parent; //父节点,weak_ptr不会增加引用计数，防止循环引用
	BTreeNode(Book&& b)
		: books(1, b), children(2, nullptr) {}
};
class result { //查找结果
	std::shared_ptr<BTreeNode> node; //节点，如果找到，node为书所在的节点，否则为应该插入的位置；如果为nullptr，说明树为空
...
private:
		...
		if (root == nullptr) {
			...
			while (curr != nullptr) {
				...
				for (; i < curr->books.size(); i++) {
					...
					else if (curr->books[i] > bid) { //只要找到比bid大的书，就可以确定bid不在这个节点，而且只可能在curr->children[i]中
						curr = curr->children[i];
						break;
					}
				}
				//此时curr可能已经变成了curr->children[i]。
				//但是如果i==curr->books.size()，则说明bid比curr->books中的所有书都大，所以只可能在curr->children[i+1]中
	...
	bool insert(Book b, int number) { //当且仅当书号相同但书的其他信息不同时插入失败，返回false
		//插入书
		...
		if (r.node == nullptr) { //空树
			...
			if (b.beConsistentwith(r.node->books[r.index])) //书号相同，书的其他信息也相同
			{
			...
			if (r.node->books.size() == 3) {
				std::shared_ptr<BTreeNode> OverweightNode(r.node);
				auto newNode = std::make_shared<BTreeNode>(OverweightNode->books.back()); //将最后一个书作为新节点的书
				OverweightNode->books.pop_back(); //删除最后一个书
				auto parent = OverweightNode->parent.lock(); //获取父节点
				do {
					if (parent == nullptr) { //根节点
						parent = std::make_shared<BTreeNode>(OverweightNode->books.back()); //将最后一个书作为新节点的书
						OverweightNode->books.pop_back(); //删除最后一个书
						...
						parent->books.insert(parent->books.begin() + r.index, OverweightNode->books.back()); //将最后一个书插入父节点
						OverweightNode->books.pop_back(); //删除最后一个书
						parent->children.insert(parent->children.begin() + r.index + 1, newNode); //将新节点插入父节点
					}
	...
	bool DeleteBook(int bid) {
		...
		if (r.node->children.empty()) { //终端节点
			...
			r.node->children.erase(r.node->children.begin() + r.index + 1); //删除孩子
			if (r.node->books.size() == 0) { //该节点已经没有书了,需要从兄弟结点或双亲结点借书
				auto parent = r.node->parent.lock(); //获取父节点
				if (parent == nullptr) { //根节点
					root = nullptr;
				}
				else { //非根节点
					...
					for (; i < parent->children.size(); i++) {
						if (parent->children[i] == r.node) {
							parent->children.erase(parent->children.begin() + i);
							break;
						}
					}
					if (i == parent->children.size()) { //没找到
						return false;
					}
				}
			}
